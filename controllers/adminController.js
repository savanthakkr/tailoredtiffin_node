...
exports.addMeal = async (req, res) => {
  try {
    ...
    const params = {
      ...
      included_side_items: JSON.stringify(body.included_side_items || []),
...
exports.getMeals = async (req, res) => {
  try {
    ...
    const formatted = meals.map(m => ({
      ...
      included_side_items: normalizeJSON(m.included_side_items),
...
/* ===============================
   SIDE ITEMS CRUD
=============================== */
exports.addSideItem = async (req, res) => {
  try {
    let body = req.body.inputdata;
    let response = { status: "error", msg: "" };

    if (!body.name || body.name.trim() === "") {
      response.msg = "Side item name is required.";
      return utility.apiResponse(req, res, response);
    }
    ...
    let insert = await dbQuery.insertSingle(constants.vals.defaultDB, "side_items", insertValue);

    response.status = "success";
    response.msg = "Side item added successfully.";
    response.data = { side_item_id: insert };

    return utility.apiResponse(req, res, response);
  } catch (err) { throw err; }
};

exports.getSideItems = async (req, res) => {
  try {
    let response = { status: "error", msg: "" };

    const listQuery = `
      SELECT side_item_id, name, price, is_active, is_delete, created_at
      FROM side_items
      WHERE is_delete = 0
      ORDER BY side_item_id DESC
    `;

    const list = await dbQuery.rawQuery(constants.vals.defaultDB, listQuery);

    response.status = "success";
    response.msg = "Side item list fetched.";
    response.data = list;
...
exports.editSideItem = async (req, res) => {
  try {
    let response = { status: "error", msg: "" };
    let body = req.body.inputdata;

    if (!body.side_item_id) {
      response.msg = "Side item ID is required.";
      return utility.apiResponse(req, res, response);
    }
    ...
    const sideItem = await dbQuery.fetchSingleRecord(
      constants.vals.defaultDB,
      "side_items",
      `WHERE side_item_id=${parseInt(body.side_item_id)} AND is_delete=0`,
      "side_item_id"
    );

    if (!sideItem) {
      response.msg = "Side item not found.";
      return utility.apiResponse(req, res, response);
    }
    ...
    await dbQuery.updateRecord(
      constants.vals.defaultDB,
      "side_items",
      `side_item_id=${parseInt(body.side_item_id)}`,
      updateValue
    );

    response.status = "success";
    response.msg = "Side item updated successfully.";

    return utility.apiResponse(req, res, response);
  } catch (err) { throw err; }
};

exports.deleteSideItem = async (req, res) => {
  try {
    let response = { status: "error", msg: "" };
    let body = req.body.inputdata;
    ...
    response.status = "success";
    response.msg = "Side item deleted successfully.";

    return utility.apiResponse(req, res, response);
  } catch (err) { throw err; }
};

exports.toggleSideItemStatus = async (req, res) => {
  try {
    const body = req.body.inputdata;
    ...
    const setCondition = `is_active=${isActive}, updated_at='${req.locals.now}'`;
    const whereCondition = `side_item_id=${parseInt(body.side_item_id)}`;
    ...
    return utility.apiResponse(req, res, {
      status: "success",
      msg: `Side item ${isActive === 1 ? "activated" : "deactivated"} successfully.`
    });

  } catch (err) {
    console.error("Toggle Side Item error:", err);
    return utility.apiResponse(req, res, {
      status: "error",
      msg: "Internal server error"
    });
...
exports.getKitchenSummary = async (req, res) => {
  try {
    ...
    for (let r of rows) {
      ...
      /* 🥛 BUTTERMILK */
      /* 🥗 SIDE ITEMS (dynamic) */
      const mealSideItems = safeJSON(r.included_side_items, []);
      for (let sideId of mealSideItems) {
        countMap.side_item[Number(sideId)] =
          (countMap.side_item[Number(sideId)] || 0) + mealQty;
      }
    ...
    // 🥛 Side Items (dynamic)
    for (let id in countMap.side_item) {
      const numId = Number(id);
      if (isNaN(numId)) continue;

      const si = await dbQuery.fetchSingleRecord(
        constants.vals.defaultDB,
        "side_items",
        `WHERE side_item_id=${numId}`,
        "name"
      );
      ...
      if (si) {
        result.push({
          ...
          id,
          name: si.name,
          total_qty: countMap.side_item[id]
        });
      }
    }
...
ORDER BY o.order_id ASC
    ...
    for (let r of rows) {
      ...
      ================================================= */
        ...
        // 🥛 Side Items (dynamic)
        const mealSideItemIds = safeJSON(r.included_side_items, []);
        let sideItemTotals = [];
        for (let sideId of mealSideItemIds) {
          const si = await dbQuery.fetchSingleRecord(
            constants.vals.defaultDB,
            "side_items",
            `WHERE side_item_id=${Number(sideId)}`,
            "side_item_id, name"
          );
          if (si) {
            sideItemTotals.push({
        ...
        if (sideItemTotals.length > 0) {
          totals.side_items = sideItemTotals;
        }
