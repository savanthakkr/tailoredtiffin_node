...
const { checkActiveSubscription } = require('./subscriptionController');
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    const has_active_subscription = await checkActiveSubscription(user.user_id);
...
function isSlotOpen(slot, setting) {
  ...
  console.log("LUNCH CUTOFF =>", setting.lunch_cutoff);
  console.log("DINNER CUTOFF =>", setting.dinner_cutoff);
...
exports.createOrder = async (req, res) => {
  try {
    ...
    for (let date of delivery_dates) {
      const allowed = isSlotOpen(slot, setting);

      if (!allowed) {
        return res.status(404).json({
          status: "error",
          msg: `${slot} orders closed`
        });
      }
    }
