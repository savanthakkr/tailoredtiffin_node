...
exports.updateRecord = async function (db, table, condition, newValue) {
	// Validate that condition doesn't contain undefined values
	if (!condition || condition.includes('undefined')) {
		console.error('SECURITY ERROR: Undefined value in WHERE clause:', condition);
		throw new Error('Invalid condition: WHERE clause contains undefined values');
	}
